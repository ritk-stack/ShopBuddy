...
class AgenticRAG:
    ...
    def __init__(self):
        ...
        self.mcp_client = MultiServerMCPClient(
            {
                "product_search": {
                    "command": "venv_new\\Scripts\\python.exe",
                    "args": ["-m", "prod_assistant.mcp_servers.product_search_saver"],
                    "transport": "stdio"
                }
            }
        )
    ...
    async def async_init(self):
        print("Initializing MCP Client and connecting to servers...")
        try:
    ...
    def _get_original_query(self, state: AgentState) -> str:
        for msg in reversed(state["messages"]):
            if msg.content != "TOOL: retriever":
                return msg.content
        return ""
    ...
    async def _web_search(self, state: AgentState):
        ...
        query = state["messages"][-1].content # Rewriter output
    ...
    def _grade_documents(self, state: AgentState) -> Literal["generator", "rewriter"]:
        print("--- GRADER ---")
        question = self._get_original_query(state)
